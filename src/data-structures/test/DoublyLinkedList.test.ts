...
import { DoublyLinkedList, useDoublyLinkedList } from '@/data-structures/DoublyLinkedList'
import { useDoublyLinkedListItem } from '@/data-structures/DoublyLinkedListItem'
...
function useLinkedListPrefilled(): DoublyLinkedList<number> {
  const linkedList = useDoublyLinkedList<number>()
...
describe(`${DoublyLinkedList.name}`, () => {
  ...
  it(`Should create proper instance of ${DoublyLinkedList.name}`, () => {
    let linkedList = useDoublyLinkedList<number>()
    ...
    expect(linkedList.headItem!.prevItem).toBeNull()
    ...
    // Insert some items
    const newItem = useDoublyLinkedListItem<number>(SIX_VAL)
